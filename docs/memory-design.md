...
UI/API では `/api/memories/recent`, `/api/memories/search`, `/api/memories/{memory_id}` に加え、GPT-safe export対象だけを見る `/api/memories/exportable` を提供します。Aster UI の Memory view は通常表示で `gpt_summary` / `summary` を優先し、`body` はread-only詳細欄に限定して表示します。
