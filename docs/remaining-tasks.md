...
- [x] **UI用 read-only FastAPI Gateway (UI-1)**:
  ...
  * `sora-secretary-api.service` user systemd service として8787へ常駐化済み。`/api/memories/recent`, `/api/memories/exportable`, `/`, `/memory` が200 OK。SSH tunnel越しのheadless Chromeで `/#/memory` を開き、Memory 2件のタイトルとpolicy badge相当の表示も確認済み。
  * Mint側に `/home/okota/bin/sora-secretary-open-memory.sh`, `sora-secretary-restart.sh`, `sora-secretary-status.sh` とデスクトップlauncherを作成済み。
- [ ] **専用UIの実データ接続 (次フェーズ)**:
  ...
  * 次の優先は `aster-ui/dist/` 同期手順の運用化、service health確認、`DailyView` の read-only 接続、または write API フェーズの検討。
