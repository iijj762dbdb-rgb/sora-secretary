...
- `/todo_add`: 新しいToDo（タスク）を追加する
- `/todo_list`: ToDo一覧を表示する
- `/todo_done`: 指定したToDoを完了状態にする
- `/todo_show`: 指定したToDoの詳細を表示する
- `/remind_add`: 新しいリマインダーを追加する
- `/remind_list`: 待機中のリマインダー一覧を表示する
- `/remind_done`: リマインダーを完了(sent)状態にする
- `/remind_cancel`: リマインダーをキャンセル状態にする
- `/briefing`: 現在のタスク・記憶・リマインダーを元に今日の状況を整理します
...
4. **ToDo管理 (Read-only および追加)**: 「タスク一覧」で `/todo_list` を直接実行します。「これToDoに入れて: 〇〇」等でタスクの追加も行えます。「todo_xxx を完了」で明示的な完了指示が行えます（曖昧な完了指示は候補提示に留めます）。
